...
## Subtasks
- [x] 3.1 Add `appLanguage` to `SettingsStore`.
- [x] 3.2 Add `AppModel` proxy access to the language setting.
- [x] 3.3 Add a language picker to the configuration modal.
- [x] 3.4 Apply selected locale to the shell view environment.
- [x] 3.5 Add persistence and UI-state tests.
- [x] 3.6 Confirm the fixed-width settings modal handles both language labels.
...
## Tests
- Unit tests:
  - [x] Missing `appLanguage` defaults to English.
  - [x] Saved `pt-BR` value loads as Brazilian Portuguese.
  - [x] Invalid persisted raw value falls back to English.
  - [x] Setting `appLanguage` writes the expected raw value.
  - [x] `AppModel.appLanguage` proxies `SettingsStore.appLanguage`.
- Integration tests:
  - [x] Selected language can drive localized UI text in a representative view/model path.
